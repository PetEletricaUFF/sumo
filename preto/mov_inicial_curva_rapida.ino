...
void CALCULA_ERRO(void) {
  ...
  if (distancia_left < LIMIAR) {
    if (distancia_right < LIMIAR) {
      error = 0;    // inimigo no centro
    } else {
      error = -1;   // inimigo à esquerda
    }
  } else if (distancia_right < LIMIAR) {
    error = 1;      // inimigo à direita
  } else if (error_antg <= 0) {
    error = -2;     // estava à esquerda, agora perdido
  } else if (error_antg > 0) {
    error = 2;      // estava à direita, agora perdido
  }
}
...
void SET_MOTORS(int PWM_ESQ, int PWM_DIR) {
  ...
  if (PWM_ESQ > 0) {
    if (PWM_ESQ > MAX_PWM) PWM_ESQ = MAX_PWM;
    OCR1A = PWM_ESQ;
    PORTD &= ~(1 << ESQ_IN1_PIN);
    PORTB |= (1 << ESQ_IN2_PIN);
  } else {
  ...
  if (PWM_DIR > 0) {
    if (PWM_DIR > MAX_PWM) PWM_DIR = MAX_PWM;
    OCR1B = PWM_DIR;
    PORTC |= (1 << DIR_IN1_PIN);
    PORTC &= ~(1 << DIR_IN2_PIN);
  } else {
